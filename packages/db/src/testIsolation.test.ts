...
describe("migration creates tables in isolated schema", () => {
...
describe("seed works in isolated schema", () => {
...
describe("setupIsolatedTestDb", () => {
  ...
  it("can run migration and seed in the created schema", async () => {
    const { db } = await createDatabase(testDb.databaseUrl, testDb.schemaName);
