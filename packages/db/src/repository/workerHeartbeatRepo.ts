...
export function createWorkerHeartbeatRepo(db: Database) {
  /**
   * Atomically upserts a heartbeat record for the given worker name + instance
   * ID. Uses INSERT ... ON CONFLICT (worker_instance_id) DO UPDATE for true
  ...
  async function upsert(
    ...
    const [row] = await db
      ...
      .onConflictDoUpdate({
        target: workerHeartbeats.workerInstanceId,
        set: {
          workerName: input.workerName,
          lastPollAt: input.lastPollAt,
          lastSuccessAt: input.lastSuccessAt ?? null,
      ...
      .returning();
    return row as WorkerHeartbeatRow;
  }
  ...
  async function findLatestByName(
    ...
    const rows = await db
      ...
      .orderBy(desc(workerHeartbeats.lastPollAt))
      .limit(1);
