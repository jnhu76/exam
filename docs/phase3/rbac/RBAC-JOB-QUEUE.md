...
- **Pre-existing test failures (NOT caused by RBAC work)**: `audit.test.ts > filters by inclusive date range` (date-boundary flake) and `gradingQueue.test.ts > lists an attempt in the grading queue` (worker-DB data ordering) both fail identically on the clean baseline (commit b4a0c13, all RBAC work reverted). Re-confirmed post-review-fixes. Recorded for the reviewer; out of scope for this PR.
