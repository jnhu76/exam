...
| Missing admin recovery path | No reset-password script found. | Production lockout recovery relies on weak/default seed or manual DB edits. | PR 3 | Resolved by PR4: bootstrap-admin + reset-admin-password scripts exist. |
...
| Phase 2 endpoints exposed in current runtime | queue endpoint uses in-memory state (resets on restart); archive endpoint is functional but Phase 2; restore endpoint is Phase 1 disrupted recovery. | Queue/archive are not required for Phase 1 happy path. | PR6 | Partially — queue/archive endpoints still exist but are not Phase 1 product paths; restore is backend recovery only. |
| RequestId/logging incomplete | Route-local errors omit requestId; logger has no standard fields/redaction. | Poor diagnosis and sensitive logging risk. | PR 6 | Resolved by PR7: requestId in all error responses; pino with redaction. |
| E2E disabled in CI | `.github/workflows/ci.yml:83-86`. | Phase 1 acceptance signals are not blocking. | PR 7 | Resolved by PR8: blocking E2E with Playwright, server.log, artifacts. |
| Save/submit/grading concurrency needs blocking proof | Implementation uses row locks (FOR UPDATE), idempotent submit, crash-recovery grading. PR6 verified via existing concurrency tests. E2E blocking CI is PR8 scope. | Concurrency verified at integration level; E2E blocking deferred. | PR6, PR8 | Resolved by PR8: blocking E2E covers happy path + resume + submit-flush. |
