...
| E12 | Pool shape: postgres.js runtime pool created with NO options → max=10, connect_timeout 30 s, idle_timeout null (no idle close), max_lifetime randomized ~30–60 min per connection (3.4.9 default fn `60 × (30 + Math.random() × 30)` s), no env override; ALL work (requests + 5 loops + readiness) shares it | `packages/db/src/postgres.ts:32-34`; settings sweep; postgres.js 3.4.9 defaults | CODE | high | D3/D12 input |
| E13 | Readiness probe: 2 s `Promise.race` budget; losing ping is NOT cancelled (as-built: the path awaits a Drizzle select and never retains the underlying postgres.js `Query` handle, so the driver's native `Query.cancel()` — present in 3.4.9, best-effort — is not wired; postgres.js also has no per-query timeout option); no single-flight/dedup of HTTP probes; monitor tick 15 s internal probe | `operabilityMonitor.ts:27,93-110`, `apiSurface.ts:80-100`, `systemStatsRepo.ts:122-129`; postgres.js 3.4.9 `query.js` + README | CODE | high | D12 residual definition |
