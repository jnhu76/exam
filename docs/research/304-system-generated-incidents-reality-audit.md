...
**CONCURRENT_DETECTOR_CONVERGENCE: attempt-row FOR UPDATE ⇒ one episode; deterministic operationId + existing op-unique ⇒ one incident. No advisory locks, no process-memory coordination.**
...
Implementation shape (the only freeze-sanctioned one): the heartbeat cycle's bounded reconciliation leg (§6) re-derives candidate work from the durable ledger — heartbeat-detected episodes (`detection_source="heartbeat_timeout"`) with NO row in `exam_incident_interruption_links` (any creator, human or System) — and invokes the System create command idempotently per episode; the F4 episode-derived operationId makes every retry converge to the same single incident.

**Human-first corner (review round 2, frozen):** inside the crash window a human may link the episode to a human-created incident (`POST /admin/incidents/:incidentId/interruptions`, `incidents.admin.ts:1216-1226`). The reconciliation leg then SKIPS that episode: ANY existing link completes the episode for #304's purposes, so one disruption never yields a second incident (a duplicate case would be uncleanable — ADR-014 §17 forbids unlinking), and F4A's terminal condition holds in every ordering.
...
- Residual nuance flagged for product sign-off: if Admin *dismisses* incident for episode E1 and E1 is later `restored` and a new episode E2 starts, (B) yields a new System incident. If product wants "dismissal = never again for this attempt", that is a stricter policy requiring an attempt-level suppression record — **PRODUCT_DECISION_REQUIRED only for this stricter variant**; default freeze is (B).
...
- **F4A — Durable completion**: a committed heartbeat-detected episode remains reconcilable until it is durably linked to exactly ONE incident — System-created, or human-created if a human linked the episode first; the reconciliation leg skips any episode that already has ANY `exam_incident_interruption_links` row (human-first corner, §8.6), so no disruption ever yields a duplicate case. Completion authority = the durable episode ledger, re-read each heartbeat cycle by the bounded reconciliation leg (§6, §8.6) — never scanner process memory, never the one-shot `marked` return value. Implementation evidence must cover C1 (crash before incident → restart → one incident), C2 (lost result → replay returns the SAME incident), C3 (concurrent reconcilers → one incident/link), C4 (human links the episode during the crash window → reconciliation skips → exactly one incident, human-created).
