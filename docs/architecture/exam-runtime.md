...
- **需求驱动**：join/status/start 三条 canonical 路径上按需 reconcile（CAS 写 `admitted_at`，幂等），**无后台 scheduler/worker**。批次时刻是**资格边界**（eligibility boundary），不是服务器自主物化的承诺：已断线考生的 `admitted_at` 可以保持 NULL，直到该考生的下一次权威交互（queue 轮询或 start）按 durable 事实重算资格并补物化。`admitted_at` 记录的是物化/对账时间（≈ 首次权威交互时刻），不是理论放行时刻，也不是 SLA/排序/截止权威。停轮询期间积压的资格会在恢复轮询时一次性收敛（需求触发的 resume burst）——这是准入容量契约的组成部分，由 #550 的 admission workload 以真实 HTTP 栈复验。
