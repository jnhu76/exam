...
| INV-I-004 | (PROPOSED, ADR-014) Every incident link (operator action, affected attempt, interruption episode) and the combined grant+link path MUST satisfy the scope triple — same organization, same exam, and `incident.attemptId` null-or-matching — derived server-side from the authoritative target row; organization and exam are NEVER taken from the request body; composite FKs reuse existing uniques (`exam_attempts_org_id_unique`, `attempt_interruptions_org_attempt_id_unique`) to enforce organization/attempt consistency at the DB level. |
| INV-I-005 | (PROPOSED, ADR-014) `exam_incident_events.event_sequence` (`BIGINT GENERATED ALWAYS AS IDENTITY`) is the sole ordering authority for reconstruction and display; state events carry `before_version`/`after_version`, the per-incident version chain MUST be contiguous and verifiable, and the materialized `version` equals the latest `after_version`; wall-clock timestamps and UUID ids define no causal order. |
...
| **Idempotency / conflict** | Every write carries an `operationId` (client UUIDv4; retries reuse it), arbitrated by `UNIQUE (organization_id, operation_id)` on `exam_incident_events`: same `operationId` + same command + same canonical payload → idempotent replay (outcome `idempotent_replayed`); same `operationId` + different command/payload → 409 `IDEMPOTENCY_CONFLICT`; new `operationId` + stale `expectedVersion` → 409 `INCIDENT_VERSION_CONFLICT`; new `operationId` + invalid transition (including any on a terminal incident) → 409 `INVALID_STATE_TRANSITION`. There is no reason-text equivalence rule. `expectedVersion` is required only on investigate/severity/resolve/dismiss; append-only commands (create, note, links) carry none. Notes are append-only and concurrent-safe, each under its own `operationId`; display order is `event_sequence` |
| **Security invariants** | INV-I-001 (orthogonality), INV-I-002 (terminal monotonicity + type immutability + operationId/version), INV-I-003 (incident-only locking), INV-I-005 (event_sequence ordering + verifiable version chain); no candidate-facing API; note/description bodies are never duplicated into audit metadata |
...
| **Preconditions** | Link-scope triple derived server-side from the authoritative action/Attempt row (never from the request): `incident.organizationId == action.organizationId`, `incident.examId == action.examId`, and `incident.attemptId` null or matching the action's attempt; the action row exists (retroactive) or is created atomically (combined); action type ∈ `time_grant` / `force_submit` |
...
| **Transaction boundary** | Combined grant+link runs inside the existing time-grant transaction (Enrollment → Attempt → Exam lock order unchanged; the incident row is validated without a lock, exam derived from the locked Attempt); retroactive link locks only the incident row |
| **Idempotency / conflict** | Every link carries an `operationId`; `UNIQUE (organization_id, action_type, action_id)` — one action links to at most one incident; duplicate under the same `operationId` + same payload → idempotent replay; duplicate under a new `operationId` (including a same-incident re-link) → 409 `INCIDENT_ACTION_ALREADY_LINKED` (23505 on the named constraint); the combined path keeps the grant's `operationId` idempotency (`granted` / `idempotent_replay` / `terminal`) |
| **Security invariants** | INV-I-001: the link is correlation, not authority — incident resolution never triggers, modifies, or reverses the action; INV-I-003: no Attempt/Exam lock is added to incident commands; INV-I-004: scope triple server-derived, composite FKs enforce organization/attempt consistency; INV-I-005: event ordering |
...
| **Preconditions** | The incident is exam-wide (`attemptId` null) — anchored incidents reject membership links with 409 `INVALID_STATE_TRANSITION` (anchor exclusivity); the attempt belongs to `incident.examId` (server-derived); `relationshipType` ∈ `affected` / `referenced` |
...
| **Transaction boundary** | Scope validation (non-locking) → insert junction (unique arbiter) + event + audit; only the incident row is locked |
| **Idempotency / conflict** | Every link carries an `operationId`; `UNIQUE (incident_id, attempt_id)`; duplicate under the same `operationId` + same payload → idempotent replay; duplicate under a new `operationId` → 409 `INCIDENT_ACTION_ALREADY_LINKED` (the code covers all three link uniques); no unlink, no bulk-link (a batch is N operationId-keyed calls) |
| **Security invariants** | INV-I-001 (evidence, not authority), INV-I-003 (incident-only locking), INV-I-004 (scope triple + composite FK reusing `exam_attempts_org_id_unique`), INV-I-005 (event ordering) |
...
| **Preconditions** | The episode's attempt belongs to `incident.examId` (server-derived); anchored incidents require `episode.attemptId == incident.attemptId`; `interruptionId` and `incidentId` are never interchangeable (ADR-013) |
...
| **Transaction boundary** | Scope validation (non-locking) → insert junction (unique arbiter) + event + audit; only the incident row is locked |
| **Idempotency / conflict** | Every link carries an `operationId`; `UNIQUE (incident_id, interruption_id)`; duplicate under the same `operationId` + same payload → idempotent replay; duplicate under a new `operationId` → 409 `INCIDENT_ACTION_ALREADY_LINKED` (the code covers all three link uniques) |
| **Security invariants** | INV-I-001 (evidence, not authority; the ledger stays authoritative for compensated interruptions), INV-I-003 (incident-only locking), INV-I-004 (scope triple + composite FK reusing `attempt_interruptions_org_attempt_id_unique`), INV-I-005 (event ordering) |
