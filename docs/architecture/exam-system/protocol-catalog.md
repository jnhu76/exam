...
## Protocol: Incident Authority (TARGET — ADR-014 ACCEPTED — NOT IMPLEMENTED)

Status: TARGET — designed by [ADR-014](../../adr/ADR-014-exam-incident-authority.md) (ACCEPTED, runtime NOT IMPLEMENTED); J3 (`REC-I6-I1-INCIDENT-PERSISTENCE-COMMANDS`) is the next authorized Job. The entries below are the proposed contracts that J3 will implement; they are not live protocols. The architecture projection (state diagram, command inventory, permission matrix, sequences) lives in [incident-authority.md](./incident-authority.md).
