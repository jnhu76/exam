...
export function trackExamEvent(
  ...
  if (THROTTLED_NAMES.has(name)) {
    ...
    if (existing) {
      ...
      existing.timer = setTimeout(() => flushCoalesced(key), DEDUP_WINDOW_MS);
...
/**
 * Cleans up all pending coalesced events for a given attempt, clearing timers
 * to prevent memory leaks. Call this when an exam session unmounts.
 */
...
/** Test-only: flushes all pending coalesced events and clears timers/state. */
