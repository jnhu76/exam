...
| `body` | Default running UI text. | family `font.ui`, size `sm` (15px), weight `400`, color primary text, leading `snug` | `body`; general paragraph text |
...
| `metric` | A KPI/stat numeric value; the prominent number of a stat card. | family `font.ui`, size `28px/34px`, weight `700`, color primary text, `font-variant-numeric: tabular-nums` | `StatsCard` value; `ExamDetail`/`ScoreList`/`SystemDiagnostics`/`AttemptDetail` stat numbers |
| `metric-hero` | The single largest numeric emphasis on a screen — a final exam score shown to a candidate. | family `font.ui`, size `48px`, weight `700`, color primary text, `font-variant-numeric: tabular-nums` | `ResultPage` candidate-facing total score |
...
| `metric-hero` | font-family, font-size, line-height, font-weight, color, font-variant-numeric | — |
