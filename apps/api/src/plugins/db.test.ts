...
describe("db plugin: P0-2 uses runtimeConfig.database.url", () => {
  ...
  // Per-mode URL RESOLUTION is owned by runtimeConfig's "database URL
  // resolution" matrix. One wiring row here proves the plugin forwards the
  // resolved URL and closes the pool; the ci row is the wiring witness (it
  // also covers the DATABASE_URL-unset edge and the close call).
