...
// ── P4-2B: capability cutover — Teacher authoring proof (task 7.3) ──
// The question routes flipped from requireRole(["Admin"]) to requireCapability.
// Teacher preset grants all 5 question perms, so Teacher must PASS the create
// gate and persist a real question. Candidate has no question perm -> denied.
// Admin has no regression (still 201). This is the write-side complement to
// the GET-only permission matrix in permissionMatrix.test.ts.
describe("question routes — P4-2B capability cutover (Teacher authoring)", () => {
  let ctx: Awaited<ReturnType<typeof buildTestApp>>;
  let courseId: string;
  ...
  beforeAll(async () => {
    ...
    // Seed a Teacher + a Candidate in the default org.
  ...
  it("Teacher creates a text_response question with a rubric (passes the create gate)", async () => {
    const res = await ctx.app.inject({
      method: "POST",
      url: "/api/questions",
      payload: {
  ...
  it("Teacher lists questions (passes the view gate)", async () => {
    const res = await ctx.app.inject({
      method: "GET",
      url: "/api/questions",
      cookies: { "auth-token": teacherToken },
  ...
  it("Candidate is denied question create at the capability gate (403)", async () => {
    const res = await ctx.app.inject({
      method: "POST",
      url: "/api/questions",
      payload: {
  ...
  it("Admin has no regression (still creates a question, 201)", async () => {
    const res = await ctx.app.inject({
      method: "POST",
      url: "/api/questions",
      payload: {
